...
%A programmer writing a tool to debug the data corruption problem in an object would like to target only memory of specific kinds (e.g. \texttt{sk\_buff}, \texttt{inode} ), and allow instrumentation explicitly to the targeted memory.
%mentioning debugging tasks like data race detection, memory usage bugs, or performance debugging such as finding false sharing hotspots are all naturally data-centric.
%
...
Third, existing DBT systems instruments everything to give comprehensive coverage, but introduces significant overheads for realistic instrumentation. If a bug occurs because the \texttt{i\_flags} field of one of the \texttt{inode} structure is corrupted then instrumenting every memory access in the kernel is excessive. In practice, we would like to instrument only the code that operates on \texttt{inode} structures.
 ...
 %are unable to instrument only the portion of code responsible for the bug. For example, if a bug occurs because the \texttt{i\_flags} field of one of the \texttt{inode} structure is corrupted then instrumenting every memory access in the kernel is excessive. In practice, we would like to instrument only the code that operates on \texttt{inode} structures.


%Third,  instruments everything to give comprehensive coverage, but at high overhead costs.



%; we get only the instructions, memory addresses, and read/write information, but can't easily connect that to higher-level abstractions like fields in data structures.



%specific data structure (e.g. \texttt{sk\_buff}, \texttt{inode} ) or log accesses to its specific fields would require inspecting all the object of the type


%need to manage these informations specifically.
...
%log accesses to specific fields, or detect invarient violation in a data structure, these information is not directly available in DBT systems.



%specific data structure, log accesses to specific fields, or detect invarient violation in a data structure, need to manage these informations specifically. They are not directly available with DBT systems.

%existing DBT provides code-centric instrumentation, but for many interesting debugging tools, we would prefer to have data-centric instrumentation

%First, DBT abstractions are too low-level and individual instructions only reveal what memory addresses are being accessed. This is problematic for creating efficient debugging tools that 
