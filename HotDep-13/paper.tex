...
\maketitle
\subsection*{Abstract}
Finding, understanding, and fixing bugs in an operating system is challenging. DBT system provides a powerful facility for building program analysis and debugging tools. However, DBT abstractions are tool low-level and provide limited contextual information at instrumentation.

In this paper, we introduce \emph{behavioural watchpoints}: a new software-based watchpoint framework that simplifies the implementation of DBT-based program analysis and debugging tools. Behavioural watchpoints extend the traditional approach of using DBT-system by providing context-specific information at instruction-level and specialising instrumentation to individual data-structure.  We describe four applications developed using watchpoint framework: detecting buffer overflows, detecting read-before-write and memory freeing bugs, detecting memory leaks and enforcing fine-grained memory access policies.

We implemented behavioural watchpoint framework using Granary, a DBT framework and show the reasonable overheads for their intended use in analysing and debugging kernel modules.
%Finding, understanding, and fixing bugs in an operating system is challenging. DBT system provides a powerful facility for building program analysis and debugging tools. However, DBT abstractions are tool low-level and provide limited contextual information at instrumentation.
%Data watchpoints are a powerful tool that can help with this problem; however, hardware-based watchpoints are too scarce for many uses and software-based watchpoints lack contextual information about the watched memory.

%In this paper, we introduce \emph{behavioural watchpoints}: a new software-based watchpoint framework that simplifies the implementation of DBT-based program analysis and debugging tools. Behavioural watchpoints extend the traditional approach of using DBT-system by providing context-specific information at instruction-level and specialising instrumentation to individual data-structure. We describe four applications developed using watchpoint framework: detecting buffer overflows, detecting read-before-write and memory freeing bugs, detecting memory leaks and enforcing fine-grained memory access policies.

%We show that behavioural watchpoint framework, as implemented using a kernel DBT-system, have reasonable overheads for their intended use in analysing and debugging kernel modules.


%watchpoints in several novel ways: they maintain context-specific information about watched memory, they watch ranges of addresses, and they invoke programmer-defined functions when watched memory is accessed. We describe three applications of these watchpoints: detecting buffer overflows, detecting read-before-write and memory freeing bugs, and enforcing fine-grained memory access policies.
...
First, DBT abstractions are too low-level and individual instructions only reveal what memory addresses are being accessed. This is problematic for creating efficient debugging tools that detects corruption in a particular field of all data structures of a specific type. DBT systems don't provide information about what data structures are accessed, only what instructions are accessing what memory addresses
...
We have built a detector for memory leaks caused by modules~\cite{Boehm96simplegarbage-collector-safety}. Our approach is based on performing reachability analysis on module-owned objects. %The challenges involved in developing leak detector makes behavioural watchpoint a natural fit.
