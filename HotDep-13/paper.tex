...
We built a memory leak detector for module-owned objects, where we say that an object is owned by a module if that module deallocates the object. Not all module-owned objects are allocated by modules. For example, \texttt{sk\_buff} objects used by network modules are allocated by kernel interrupt handlers, but must be deallocated by modules. Our approach ensures that all module-owned objects are watched when allocated.
...
\begin{figure}
%\subfloat{
\epsfig{file=watchpoint.eps,width=2.5in}
 %}
\caption{\label{fig:iozone-workload-overhead} Filesystem module (\texttt{ext3}) I/O performance at different workloads (write workloads (w), read workloads (r)).}
\end{figure}
