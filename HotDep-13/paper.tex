...
\paragraph*{IOzone}  In our experimental setup, \texttt{ext3} filesystem module was mounted on RAMDisk of size 1GB and IOzone creates two processes (reader, writer) performing common file operations on to a file of size 480MB with record size of 1Kb. We wrapped two most used allocators by \texttt{ext3} \& \texttt{jbd} (e.g. \texttt{\_\_kmalloc} and \texttt{kmem\_cache\_alloc}) to add watchpoints at allocated objects. As shown (\Figref{iozone-workload-overhead}), the watchpoint framework 
