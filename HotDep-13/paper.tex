...
Program debugging is a tedious and time-consuming part of software development. Detecting software bugs such as data corruption, bad pointers, and data races require developers to manually inspect millions of instructions that may modify data. The advent of multicore system and the ever increasing complexity and size of software has made program debugging even more challenging. 

Dynamic binary translation (DBT) systems provide a powerful facility for building debugging and analysis tools because they allow instrumenting the entire program at an instruction granularity~\cite{DynamoRIOKernel}. For example, Valgrind's Memcheck \cite{Seward:2005:UVD:1247360.1247362} and Helgrind \cite{Muehlenfeld:2007:FDM:1229428.1229457} uses binary translation to detect common memory errors (e.g. use-after-free, read-before-write, memory leaks) and threading bugs (e.g. data races) in user space programs.
%Valgrind's Memcheck \cite{Seward:2005:UVD:1247360.1247362} uses binary instrumentation to detect common memory errors (e.g. use-after-free, read-before-write, memory leaks)
%threading bugs (e.g. data races) in user space programs.
%), such as buffer overflow, and Cachegrind~\cite{Nethercote:2007:VFH:1250734.1250746} helps profile program execution. 
These tools are powerful but developing them is challenging for three reasons:

First, the DBT abstractions are too low-level and individual instructions only reveal what memory addresses are being accessed. This is problematic for creating efficient debugging tools that track meta-data for each in-memory data structure (e.g. to detect if a byte was written before it was read) because this meta-data must be queried on each memory access.

Second, DBT systems are unable to specialise instrumentation to individual data structures. For example, detecting corruption in the \texttt{i\_opflags} field of kernel \texttt{inode}s requires inspecting the permissions referenced by the related \texttt{i\_op} field in each \texttt{inode}. This is challenging to do in existing DBT-based debugging tools, which view data structures as opaque sequences of bytes. In practice, we would like our tool to be able to target only memory of specific kinds, and allow instrumentation to be specialised to the targeted memory.

Existing DBT systems are unable to instrument only the portion of code responsible for the bug. For example, if a bug occurs because the \texttt{i\_flags} field of one of the kernel \texttt{struct inode} data structures is corrupted then instrumenting every memory access in the kernel is excessive. In practice, we would like to instrument only the code that operates on \texttt{inode}s to find \texttt{inode} corruption.
...
The watched addresses are non-canonical addresses that trigger a hardware exception when dereferenced. We take advantage of the x86-64 architecture for implementing watched addresses. In kernel space on x86-64, canonical addresses have their 16 high-order bits set to 1. Watched addresses do not take this form. Watchpoint framework uses two approach to perform the memory operation at watched objects. It dynamically adds instrumentation at every memory loads and stores to avoid the hardware exceptions.  Watched addresses are detected before they are dereferenced and resolved to their unwatched counterparts (by masking the 16 high-order bits to 1). The alternative approach is to dereference a watched address and implement behavioural watchpoints instrumentation in trap handler. This enables on-demand binary translation and allow adding instrumentation only when watchpoint gets triggered. This lowers the overhead of binary translation when watchpoints are fewer in number and unlikely to be triggered. 
...
\paragraph{Watchpoint descriptors are context specific.} Our design seperates the allocation and managment of descriptors from watchpoint framework. It is the responsibility of client to manage its decriptor. % and declare the corrosponding descriptor type for the framework. 
This provides complete flexibility in handling it. When a watchpoint gets added to an object, client determines \emph{type}, \emph{size}, \emph{meta-information} or \emph{vtable} which needs to be stored in the descriptor. These descriptors can be arbitrary customized or extended based on the need of client. Two powerful applications of this extension are discussed in \Secref{type_overflow} and \Secref{access_policies}.
...
\paragraph{Triggered watchpoint functions are polymorphic.} The function invoked when watched memory is accessed is decided using descriptor-specific virtual table (vtable). Each vtable provides eight functions: four read and four write functions. Each function is specific to a memory operand size (1, 2, 4, or 8 bytes). A watchpoint descriptor is initialised with either a generic or a type-specific vtable. Type-specific vtables are specific to the \emph{type} of the watched address. %Behavioural watchpoints earn their name from vtable because they allow watchpoints to behave differently when watched memory is accessed.
...
We have build a detector for memory leaks caused by modules~\cite{Boehm96simplegarbage-collector-safety}. Our approach is based on performing rechability analysis on module owned objects. The challenges involved in developing leak detector for kernel modules makes behavioural watchpoint a natural fit.

Module allocates many independent objects which they share with the core kernel. Module losses its visibility once these objects are passed to the kernel. For example, a network module during device registration can allocate an object and pass it with \texttt{net\_device} structure. The object becomes invisible to the module and gets accessed through the kernel interface \texttt{netdev\_priv}. This makes it difficult to detect leaks based on module's limited view. Bhavioural watchpoints provides a way to ensure complete visibility on memory access of these objects by enabling demand-based translation and marking them as live.

All module owned objects are not directly allocated by the module and kernel can allocate it on behalf of the module. For example, a network driver module may make a call to an skb allocator (to allocate a \texttt{sk\_buff} structure) in an interrupt context when the module code is not running. Once the buffer is send over the network, it is the responsibility of the network module to deallocate the buffer. In Granary, we use hot-patched wrappers to add watchpoints at relevant allocators (e.g., \texttt{\_\_alloc\_skb, skb\_clone}). The leak detector keeps track of these watched objects and reports possible leak. 
...
We have evaluated the performance of behavioural watchpoints framework using a microbenchmark and IOzone. Our tests run on a desktop equipped with an Intel\textregistered\ Core\texttrademark\ 2 Duo 2.93 GHz CPU with 4GB physical memory. Watchpoint instrumentation was enabled for every memory load and store and we used null client and heap-based buffer overflow detector to measure the cost of using watchpoints. Null client includes the instrumentation for dereferncing watchpoint address.
...
\begin{figure}
 %\subfloat{
\epsfig{file=watchpoint.eps,width=3.2in}
 %}
...
\paragraph{Microbenchmark} We performed a microbenchmark consisting of a tight-loop of memory operations and exibits the worse-case overhead of using watchpoint framework with null client and buffer-overflow detector. The cost of using watchpoint framework with null client was 3.8{\footnotesize$\times$} and buffer-overflow was 21{\footnotesize$\times$}. The extra cost of buffer-overflow detector comes from the instrumentation required for bounds checking.
...
\paragraph*{IOzone} We studied the disk I/O performance of filesystem module \texttt{ext3} using Iozone while running it under the control of watchpoint framework. We mounted \texttt{ext3} on ramdisk of size 1GB to avoid the disk performance bottleneck and only measure the CPU overhead. Iozone creates two reader/writer process each operating on a file of 480MB written in 1kb records. We also enabled the direct I/O to avoid the effect of buffer cache. Watchpoint framework is developed over Granary, a DBT framework. To study the performance overhread of watchpoint framework, we first evaluated the overhead caused by Granary by running it with null client. This is the minimum cost associated with all the client developed over Granary. We next evaluated the overhead of watchpoint instrumentation using a null client and compared it with the cost associated when using watchpoints. We wrapped two allocators \texttt{\_\_kmalloc} and \texttt{kmem\_cache\_alloc} to add watchpoints at the objects allocated by \texttt{ext3}. \Figref{iozone-workload-overhead} shows difference in overhead when watchpoint gets added to the object. This is because in kernel environment modules and kernel frequenctly shares object, thus many of these watched objects gets passed to the kernel. Any attempt to dereference them triggers hardware-exception and attaches watchpoint framework instrumenting part of kernel code. 
