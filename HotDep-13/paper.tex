...
We built a memory leak detector for module-owned objects. We define a module-owned object to be one that the module must deallocate. Not all module-owned objects are allocated by modules. For example, \texttt{sk\_buff} objects used by network modules are allocated by kernel interrupt handlers, but must be deallocated by the network modules. Our approach ensures that all module-owned objects are watched when allocated.
