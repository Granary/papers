...
is invoked. This function handles the state transitions of \Figref{states_of_shadow} internally using an atomic compare and %
swap.}
\end{figure}
...
already. The $CheckContention$ algorithm (\Figref{algorithm}) performs both the ``Acquire" and the ``Detect" state transitions of
\Figref{states_of_shadow} using a single atomic compare-and-swap operation (\texttt{CAS}).
