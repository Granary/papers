...
false sharing on specific types of objects. We discuss this further in \Secref{beliefs}.

\subsection{Type-specific Sampling}\label{sec:beliefs}

%First, a sampling based approach implies the potential for false-negatives. That is, \Toolname{} can report that no
%false sharing was detected in some particular code. This, however, does not imply the absense of false sharing. Merely,
%it suggests that if there is false sharing, then it was missed because the wrong data was sampled at the wrong time.



%\begin{description}
%\item[Code Hotness Analysis] \hfill \\
%Here we instrument all code to count how often each basic block is executed. The results of this analysis are used when 
%
%The purpose of this
%stage is to measure code hotness for the sake of ranking detected instances of false sharing. The accuracy of this stage depends
%on the workload used to exercise the code. If this workload does not resemble production workloads then the recorded metrics
%might rank false sharing instances incorrectly.
%
%\newcommand\TypeId{$TypeId$}
%
%\item[Type Analysis] \hfill \\
%This stage instruments all code and identifies what types of objects are accessed by what code. Object types are loosely
%defined as \TypeIdPair{} pairs, where $PC$ is the return address of an allocation function, and $size$ is the number of bytes
%being allocated. To each \TypeIdPair pair, we associate a unique \TypeId{}. This \TypeId{} is used to ``taint" allocated
%addresses \cite{AddressWatchpoints}, and when accessed, is used to taint the code accessing the data with the \TypeId{}.
%The overarching purpose of this step is to map every program counter to a set of 
%
%\end{description}

%\subsection{Code Hotness}
%\Toolname{} instruments all code and counts the number of times each basic block is executed.
