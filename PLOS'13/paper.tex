...
\DeclareCaptionFormat{ruled_caption}{#1#2#3\vspace{-0.6em}\hrulefill\vspace{-1.6em}}
...
%Our extensive use of compile-time meta-programming enables efficient, dynamic analyses that are driven by static kernel type information. 
...
%       -   No duplication across threads/cores, i.e. smaller memory footprint.
%   X)  Stateless:
...
%Our goal when creating Granary was to make it easy to create efficient, low-level analysis tools (e.g. to analyse memory accesses, instructions, basic blocks, or control flow) that integrate high-level static analysis information (e.g. figuring out which data structure fields are associated with each memory access). Granary uses dynamic binary translation (DBT) to just-in-time decode, instrument, and encode \texttt{x86-64} binary module instructions. 

%Unlike other binary analysis systems, Granary tools can use kernel static analysis information to change how code is instrumented and to provide meaning to otherwise typeless instructions/memory. 
...
%Therefore, it becomes necessary to track the execution context of each translated basic block: the first execution of the first basic block of every function is in the context of an entrypoint, and all other executed basic blocks, including re-executions of any function's first basic block, is after the context of an entrypoint. Unfortunately, existing DBT systems commit to translating every basic block in one way for one context, and cannot predict if that basic block will eventually execute within a different context.
...
%Granary easily solves the entrypoint tracking problem by allowing tools to dynamically changing the policy used to instrument code, called \emph{policy-driven instrumentation}. 


%The just-in-time nature of DBT systems and the way basic blocks are linked together makes it challenging 
%Arbitrary module pre-emption and resumption can cause the context in which a module executes 
%Implementing policy tracking and switching in a way that is convenient for tool-developers is challenging in the face of arbitrary module pre-emption and resumption.
%is challenging in the face of arbitrary pre-emption 
...
%is applied to every \texttt{call} instruction, and $P_{\mathit{exit\_function}}$ is applied to every other control-transferring instruction.
...
% One policy, $P_{\mathit{function_entry}}$ is applied to the first basic block of every function. When executed
...
%This is challenging to do in some binary instrumentation systems because the first few instructions of a function might be re-executed within the function. In Granary, we can easily solve this problem by applying one policy ($P_{\mathit{call\_entry}}$)  to every \texttt{call} instruction, and another policy ($P_{\mathit{after\_entry}}$) to every non-\texttt{call} control-flow instruction.  The effect of this application of policies is that the first basic block executed on entry to a function will be instrumented by $P_{\mathit{call\_entry}}$; however, if the same basic block is targeted by a non-\texttt{call} instruction (e.g. a backward branch) then a second version of that basic block will be instrumented by $P_{\mathit{after\_entry}}$.

%For example,  we sometimes want to implement different ``levels'' or ``weights'' of instrumentation that apply lightweight/null instrumentation to the majority of code, and heavyweight instrumentation to only some code. 
...
%Granary will ensure that the code targeted by \texttt{call} instructions will be instrumented according to $P_{\mathit{call\_entry}.
...
%, then one 
%For example, policy switching can be used to implement simple things like measuring module performance by reading the time stamp counter every $N$th basic block, or more complex things like switching from a lightweight $P_{\mathit{null}}$ policy to a heavyweight $P_{\mathit{check\_shared}}$ policy when a basic block instrumented by $P_{\mathit{null}}$ acquires a lock.
...
%The last limitation of current systems is that they do not provide static program information while instrumenting arbitrary, binary modules. This is unfortunate because static program analysis provides ``big picture'' information that is sorely missing when doing instruction-level instrumentation. 
...
%	
%	%because a thread of execution can be arbitrarily interrupted and resumed on a different CPU. 
%	
%	%The complexities of managing the existing state increased when we modified DRK to frequently take and relinquish control of execution. For example, our modified DRK once ``got lost'' because uncleared state on one CPU caused some unrelated code to redirect its execution to recently interrupted code.
%	
...
%	\end{enumerate}
%	
%	Combined, these forms of transparency limit tools to adding only idempotent instrumentation. Instrumentation instructions executed during the delay of an interrupt may be re-executed after the kernel handles the interrupt because DRK re-instruments the native code targeted by the interrupt's return address. In this model, one cannot spill/restore registers around a native instruction because the spilling instructions may be arbitrarily re-executed while the restoring instructions will execute only once.
%	
...
%It is difficult to create tools that reason about arbitrary, binary modules without incorporating available information present in the kernel's source code. This motivated the 
...
%\end{enumerate}
%If module source code is unavailable then static analysis of module binaries can range from being a chore (e.g. optimised binaries stripped of debugging information) to being intractable (e.g. indirect control-flow that jumps into the middle of variable-length instructions). Regardless of if module source code is available, static analysis is challenging because of the tight interaction between the module and the kernel.
%Dynamic analysis of kernel modules is challenging because existing tools are either too coarse grained (TODO CITE: DRK, PinOS, QEMU, KVM) or are optimised for performance at the cost of expressive/flexible tooling.
...
%\comment{!!!This might narrow the usefulness of Granary down by making it seem like we're only useful where static analysis fails, or like we're only useful when source code is unavailable.!!!}
...
%	 %up until some condition (context-, control-, or data-specific) is true, null instrumentation is used for elf
...
%		\item In \Cref{sec:interrupts}, we depart from previous kernel-space instrumentation work and discuss a new approach to handling hardware and software interrupts occurring within translated code.
%	\end{enumerate}
...
%DBT enables Granary to analyse arbitrary, binary modules because \begin{inparaenum}[i)]
%	\item 
%	\item some modules are only distributed in a binary format;
%	\item \texttt{x86} module binaries can mix data and instructions, which makes static binary analysis intractable; and
%	\item DBT c
%DBT does not depend on virtualisation, which restricts what kinds of modules can be analysed; and
%	\item static binary translation 
%on architectures like \texttt{x86}, where the mix of variably-sized instructions and data make static binary translation intractable.
%\end{inparaenum}

%Our goal of being able to analyse arbitrary, binary modules motivated our choice to

%Binary translation is attractive when few assumptions can be made about the nature of the programs being translated. DBT is necessary to analyse binaries 
...
Granary uses static analysis to dynamically discover attach points by wrapping the kernel/module interface. Kernel functions are wrapped by \emph{kernel wrappers} that inspect and traverse argument pointers in search of pointers to module functions. \Cref{fig:type_wrapper} shows an example of a type wrapper for the Linux device driver structure. If a pointer to a module function (a future attach point) is discovered, then Granary replaces that pointer with a function-specific \emph{module wrapper}. Granary inspects and modifies arguments to module wrappers in the same way as for kernel wrappers. Finally, Granary redirects execution to the appropriate kernel or instrumented module function after wrapping has occurred.
...
\paragraph{Interrupt return addresses} Kernel interrupts are not sensitive to return addresses and hence relaxing return address transparency has no effect on interrupts. A special case arises for page fault exceptions that occur within specific kernel functions that access user space data. Granary handles this special case in a similar way to the kernel. Thus, maintaining return address transparency with respect to interrupts in instrumented module code has no value.
...
%This section evaluates Granary using the \texttt{iozone} file system benchmark to exercise the \texttt{ext3} and \texttt{jbd} file system and journalling kernel modules.

 %Our experiments show that mixed-mode execution achieves significant performance improvements over whole-kernel instrumentation, and that relaxing transparency model yields better performance than can be achieved using 

%Our evaluation compares the overheads of: \begin{inparaenum}[i)]
%	\item DynamoRIO Kernel (DRK) \cite{DRK}, a whole-kernel DBT framework;
%	\item ModifiedDRK, a modified version of DRK that only instruments modules; and
%	\item Granary.
%\end{inparaenum}

We evaluated Granary by measuring throughput of common file system operations using the \texttt{iozone} file system benchmark on an \texttt{ext3} file system.
To focus our evaluation on the CPU overhead of Granary and avoid the disk performance bottleneck, we mounted \texttt{ext3} on a 1 GB Ramdisk.  The benchmarks were run on an otherwise idle desktop equipped with an Intel\textregistered\ Core\texttrademark\ 2 Duo 2.93 GHz CPU with 4GB of RAM.  In our experiment, \texttt{iozone} mounts an \texttt{ext3} file system (which installs two kernel modules: \texttt{ext3} and \texttt{jbd}) and creates two processes (reader and writer) that read from and write to a 480MB file 1KB at a time. 
...
%In our experiment, both the \texttt{ext3} and \texttt{jbd} file system and journal modules are instrumented by Granary.

% with the existing framework by modifying DRK to instrument the kernel modules.  we implemented similar attach/detach mechanism for DRK at the kernel/module interface including similar kernel wrappers for fast attach and page protection as the fall-back mechanism to regain control. 



%We evaluated Granary on the following three parameters: 1) Overhead of Granary with null-instrumentation, 2) Advantage of wrappers for fast attach/detach and 3) Advantages of mixed-mode execution in the Linux kernel. 

%Granary is designed with the motivation to allow mixed-mode execution in Linux kernel. This is particularly important to avoid the unnecessary performance overheads incurred by instrumenting the whole kernel. %Granary uses a type-rich wrapper for attach and detach of framework at kernel/module interface. Based on the design of Granary, 
...
%We ran the experiment on a desktop equipped with an Intel\textregistered\ Core\texttrademark\ 2 Duo 2.93 GHz CPU, 4GB memory, and an Intel 82598EB 10 Gigabit Ethernet controller. We used IOzone filesystem benchmark to exercise ext3 module while running it with Granary. Iozone is a useful benchmark for testing data throughput with number of different access pattern. we avoided bottleneck for disk I/O by partitioning main memory to load ext3 module. We enable Iozone to run in throughput mode with two active processes and evaluated the system under three different workloads: read, write and mixed.

\section{Related Work}\label{sec:related}
This section describes how Granary differs from related work.

\paragraph{Whole-kernel/system DBT} DRK \cite{DRK} is a kernel space port of the DynamoRIO \cite{DynamoRIO} DBT framework. DRK instruments the entire kernel, including modules. DRK follows a strict transparency model, which limits the flexibility of instrumentation. PinOS \cite{PinOS} is a port of the Pin \cite{Pin} DBT framework that performs whole-system instrumentation. PinOS depends on hardware virtualisation, which limits it to instrumenting systems that use only virtualisable modules. 

%Neither system provides mechanisms for arbitrary attach/detach, runtime code specialisation, or integration of static analysis information.

%Neither PinOS nor DRK are able to specialise instrumentation code to different execution contexts.
%PinOS is unable to perform fine-grained instrumentation and depends on hardware virtualisation, which limits it to instrumenting systems using only virtualisable device driver modules.
