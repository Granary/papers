...
Granary includes configurable levels of transparency to address the trade off between transparency and overhead. Granary defaults to being non-transparent for efficiency and flexibility, and if transparency is needed to instrument a module then transparency can be enabled at the cost of increased overheads. By default, Granary exposes the following three artifacts: code cache addresses as function and interrupt return addresses, and wrapped module function addresses in shared data structures.
...
\paragraph{Interrupt return addresses} Kernel interrupts are not sensitive to return addresses, except in the special case of page faults occuring within specific kernel functions that access user space data. Granary handles this special case in similar way to the kernel. Therefore, maintaining return address transparency with respect to interrupts in instrumented module code has no value.
