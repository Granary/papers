...
Granary detaches when control transfers from instrumented code to native (uninstrumented) code. Detaching occurs is one of three ways: \begin{enumerate}
	...
	\item {\bf Direct detaching:} instrumented module code invokes a kernel function directly. This happens when a tool marks a kernel function as not needing any wrapping, or when a tool---as part of its instrumentation---injects an unwrapped \texttt{call} or \texttt{jmp} to a kernel function. In the former case, tools must be explicit about what kernel functions should not be wrapped because Granary defaults to automatically wrapping all kernel functions.
\end{enumerate}
...
% By default, all module code is instrumented by the initial policy. However, if a tool wants to declare an execution context switch (e.g. entering into a read-critical section) then the active
