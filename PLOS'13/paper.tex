...
Attaching occurs when the kernel invokes or returns to module code. Attaching takes three forms: \begin{enumerate}
	\item Implicit attaching, where the kernel returns to instrumented module code.
	\item Fast attaching, where the kernel invokes a wrapped module function. If the kernel invokes a wrapped module function then, depending on the defined wrapping policies, the arguments to the module function may be inspected or altered by the instrumentation tool.
	\item Heavyweight attaching, where the kernel faults when trying to invoke page-protected module code. In this case, Granary handles the fault by returning execution to the instrumented version of the faulting module code.
\end{enumerate}
