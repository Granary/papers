...
Kernel and module wrappers invoke \emph{type wrappers} to find and wrap pointers to module functions that are (in)directly referenced by kernel/module function arguments (\Cref{fig:type_wrapper}). A type wrapper is a function that recursively  traverses the in-memory object graph and converts pointers to module code into pointers to wrapped module functions. Granary automatically applies the correct type wrapper in all cases using \texttt{C++} template meta-programs that operate on kernel static type information (\Cref{sec:reify}). Automating the generation of kernel, module, and type wrappers was an important goal for Granary because the kernel/module interface contains thousands of functions and data structures.
...
Granary includes configurable levels of transparency to address the trace off between transparency and overhead. Granary defaults to being non-transparent for efficiency and flexibility, and if transparency is needed to instrument a module then transparency can be enabled at the cost of increased overheads. By default, Granary exposes the following artifacts: \begin{enumerate}
