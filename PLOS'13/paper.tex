...
$F_{module}$ and $F_{kernel}$ can abitrarily manipulate instructions and policy properties before they are packaged into basic blocks. These functions decide what the ``next'' policy should be for each CTI within the basic block. For example, if $P_1$'s $F_{module}$ sets the policy of a CTI in the basic block to policy $P_2$, then any module code targeted by the changed CTI will be translated and instrumented by $P_2$'s function $F_{module}$. 
