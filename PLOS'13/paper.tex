...
The challenges and opportunities of the kernel environment motivated the creation of Granary \cite{GranaryAtOSDI,GranaryAtASPLOS}. Granary is a new binary instrumentation framework for the Linux kernel environment, designed specifically for analysing kernel modules. Granary targets modules because they represent the bulk of new bugs and new code in development in the Linux kernel \cite{FaultsInLinux}. Granary allows developers to create flexible and dynamic analysis tools for arbitrary Linux kernel module binaries. Granary has two main goals: \begin{inparaenum}[i)]
...
Granary addresses three limitations of existing program analysis systems: \begin{enumerate}
	\item They have focused on instrumenting all code (the whole kernel \cite{DRK} or whole system \cite{QEMU,PinOS}). 
	\item They do not allow for the contexts in which translated code will execute to alter how that code is instrumented \cite{Pin,DynamoRIO,QEMU}.
	\item They do not provide static type information to the instrumented code \cite{PinOS,Pin,DRK,DynamoRIO,QEMU}.
\end{enumerate} 
...
%Abitrary module pre-emption and resumption can cause the context in which a module executes 
...
%is challenging in the face of abitrary pre-emption 
...
In \Cref{sec:modes}, we describe how to efficiently implement mixed-mode execution; in \Cref{sec:policies}, we describe a new method for dynamically changing how code is instrumented; in \Cref{sec:reify}, we describe how to integrate static analysis information into a DBT system; and in \Cref{sec:eval}, we evaluate Granary's performance.
...
Granary uses dynamic binary translation (DBT) to instrument Linux kernel modules. DBT is used for instruction set translation/emulation \cite{QEMU}, runtime optimization \cite{DynamoRIOOptimisation}, and runtime instrumentation (analysis, security, debugging) \cite{Vx32,NaCl,Valgrind}. We chose DBT because \begin{inparaenum}[i)]
	\item some modules are only distributed in a binary format;
	\item static binary analysis tools are unable to cope with \texttt{x86}'s mix of variably-sized instructions and data; and
	\item virtualisation-based approaches are unable to analyse non-virtualisable device drivers.
\end{inparaenum}

%existing source code analysis tools 
%DBT enables Granary to analyse abitrary, binary modules because \begin{inparaenum}[i)]
%	\item 
%	\item some modules are only distributed in a binary format;
...
%Our goal of being able to analyse abitrary, binary modules motivated our choice to
...
Granary detaches when control transfers from instrumented code to native (uninstrumented) code. Detaching occurs in one of three ways: \begin{enumerate}
	...
	\item {\bf Unwrapped detaching:} instrumented module code invokes a kernel function directly. This happens when a tool---as part of its instrumentation---injects an unwrapped \texttt{call} or \texttt{jmp} to a kernel function.
\end{enumerate}
...
\paragraph{Function return addresses}\label{para:return_address_transparency} By default, Granary does not maintain return address transparency. Return addresses to instrumented code are exposed to instrumented functions and kernel functions. The benefit of relaxing return address transparency is efficiency: Granary will not do extra work (i.e. an indirect branch lookup) to resolve the target of a \texttt{ret} (return from procedure) instruction. If Granary is configured to use transparent return addresses, then implicit attaching and detaching are disabled. Granary will fall back to depending on slow attaching and performing an indirect branch lookup before executing \texttt{ret} instructions.

\paragraph{Interrupt return addresses} Kernel interrupts are not sensitive to return addresses, except in the special case of page faults occuring within specific kernel functions that access user space data. Granary handles this special case in a similar way to the kernel. Therefore, maintaining return address transparency with respect to interrupts in instrumented module code has no value.
...
\begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=3cm,scale=0.8, every node/.style={scale=0.8}]
  \node[initial,state]          (C)   {$P_{call\_entry}$}; 
  \node[state]          (A)[right of=C]      {$P_{after\_entry}$};
...
To implement our optimisation, we solved a more fundamental problem: tailoring instrumentation to the context in which the instrumented code will execute. Our solution was to allow different versions of the same module code to co-exist within Granary's code cache. That is, a different version of each of a module's basic blocks exists in Granary's code cache  for each encountered execution context. In the case of our RCU checker tool, there were two execution contexts: within and outside of a read-side critical section. If the same module basic block is  executed in both contexts, then Granary's code cache would contain two different instrumented versions of that basic block. The way that Granary distinguishes between different execution contexts is with \emph{instrumentation policies}. 
