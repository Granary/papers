...
Instrumenting all code (e.g. the whole kernel) imposes unnecessary performance overheads when only a subset of code the code is being analysed (e.g. modules). In practice, we would like to target instrumentation at specific module code for efficiency, while also allowing rich and detailed instrumentation of the relevant code. This requirement motivated the design of \emph{mixed-mode execution}, which allows Granary to instrument module code while non-module kernel code executes with zero overhead.
...
\begin{figure}[t]
\begin{center}
\epsfig{file=diagrams/policy-switching-entry-tracking.eps,width=1.8in}
\end{center}
\caption{\label{fig:policy_switching}Example of policy switching that ensures that some instrumentation ($P_{call\_entry}$) executes once at the beginning of a function, regardless of if the first basic block of a function is re-executed. Basic block (1) is instrumented by both $P_{call\_entry}$ and $P_{after\_entry}$, and basic blocks (2) and (3) are instrumented by $P_{after\_entry}$.}
\end{figure}
