...
DBT systems are dynamic insofar as compilation occurs at runtime instead of ahead-of-time. Granary employs a just-in-time (JIT) compilation strategy: input executable code is compiled one basic block at a time. Compilation occurs on-demand: a basic block from the input code is only translated when an attempt is made to execute that block (assuming it has not already been translated). This approach is ideal for \texttt{x86} architectures, where instructions can be overlayed or mixed with data, because Granary will ``discover'' the entrypoints to basic blocks in the input code as the program attempts to execute those basic blocks.
...
Granary extracts type and function declarations from the Linux source code and exposes that information to tool writers. This information can be used by tool-authors to inspect data at mode-switch boundaries (kernel and module wrappers) and to dynamically change the instrumentation policy used to instrument module code.
...
For example, we are actively developing a tool that uses reifying instrumentation to create models of module behaviour. Reifying instrumentation has two roles in this tool. First, we use assign kernel types to module allocated memory that is shared across the module/kernel interface. Type assignment plays a critical role because it allows us to correlate memory reads and writes within an object to the exact structure fields of that object. Second, we use the kernel and module function wrappers to generate compressed call graphs of a module's execution from the perspective of the kernel.  When combined, this information allows us to say when and where specific parts of kernel data structures are accessed.

The information derived from multiple ``trusted'' modules (e.g. mature, open-source file systems) is combined to construct models of typical module behaviour. We hope that these models will help us to automatically generate new tools that identify spurious module behaviour.
