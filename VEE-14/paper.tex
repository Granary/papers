...
%Also, the runtime checks needed to multiplex between different versions of instrumented code are orthogonal to the true goals of the instrumentation tool, thus distracting tool-developers from their primary objectives.
...
% Granary's \toolname{Irrigator} tool find leaks on module-owned memory (\Cref{sec:function_wrapper}). \toolname{Irrigator} instruments only module code, but also interposes on specific kernel functions (kernel memory allocators and deallocators) to gain visibility into kernel code that would normally execute natively (using module-only instrumentation).\\
\hline
%Basic Block & Individual basic blocks are instrumented on demand. & Granary's address watchpoints framework can be tuned to instrument only code accessing specific data. A general protection fault occurs when native code dereferences a  ``watched address". Granary can transparently handle this fault by returning execution to an instrumented version of the faulting code (where the instrumentation prevents such faults). Native execution can resume at the end of the basic block. \\
%\hline
\end{tabularx}
...
%The closest that an existing system can get to 

% Therefore, if Guardrail were implemented using DRK or btkernel, then it would have to implement its own form of runtime code specialization



%. Recording and analyzing every instruction executed by the kernel is 

%Guardrail records and analyzes traces of device driver code to find bugs. However, Guardrail's analysis can be heavyweight. If Guardrail were forced to instrument all code (e.g. with DRK or btkernel), then Guardrail would impose significant performance overheads. However, if Guardrail's recording were limited to only module code (

%and so limiting the analysis to only device driver code results in better performance. 

%The recording and analysis phases of Guardrail are heavyweight, therefore, limiting 

% Instrumenting the whole kernel to observer driver (module) execution is unnecessary and potentially very inefficient (especially if the tool is meant for online analysis). That is, 
...
%instrument all kernel code, specific module code, or only specific function code.
%what code should be instrumented. Example granularities of instrumentation include the whole kernel, only specific modules, only specific functions, or on-demand instrumentation based on events (e.g. interrupts or exceptions).
...
%As with existing modules, the native code segments of dynamically loaded modules are page protected to prevent and recover from accidental execution. 

%Granary interposes on an existing module by using hardware page protection to mark the module's code as non-executable and trapping attempts to execute the protected code. 
...
%Granary's basic attaching mechanism, page protection, raises a trap when registered functions are executed. In practice, we would like to avoid the overhead of the trap by proactively finding attach points and replacing them with their instrumented equivalents.


%Therefore, to Granary, any function pointer passed from the module to the kernel is a potential future attach point.
%registered module functions represent potential future \emph{attach} points, where a switch from native to instrumented execution will occur. 
%Furthermore, Granary discovers additional attach points at detach points by observing pointers to module functions that are passed by modules to the kernel.

%Granary's use and exposition of static analysis makes it the first kernel-space DBT system to bridge the gap between static and dynamic analysis.
...
%Any function pointer passed as an argument to a kernel function, or stored in a structure that can be passed as an argument to a kernel function is treated as a potential attach point. 
...
%\Cref{fig:type_wrapper} shows an example of a type wrapper for the Linux device driver structure.
...
% instrumentation tools a mechanism to observe a

%are responsible for wrapping their arguments. When an argument 

%At runtime, these wrappers interpose on kernel functions and traverse in-memory data structures, find attach points, and convert them into 

%At runtime, Granary detects and interposes on potential attach points by wrapping the kernel/module interface. 

%Kernel functions are wrapped by \emph{kernel wrappers} that inspect and traverse argument pointers in search of pointers to module functions.  Granary inspects and modifies arguments to module wrappers in the same way as for kernel wrappers. This allows Granary to discover kernel entry points that will cause instrumentation to detach. Finally, Granary redirects execution to the appropriate kernel or instrumented module function after wrapping has occurred.
...
%For example, Granary's \toolname{Irrigator} tool detects memory leaks in kernel modules.  \toolname{Irrigator}  finds leaks of ``module-owned" memory. We say that the some memory is owned by a module iff that memory is deallocated by a module. Unfortunately, ownership of module-allocated memory (for an arbitrary, binary module) is not known \emph{a priori}. Therefore, \toolname{Irrigator} conservatively assumes that all module-allocated memory is module-owned, unless it is deallocated by the kernel. \toolname{}


%However, the instrumentation granularity of \toolname{Irrigator} is module code, and so (almost all) kernel code executes natively. Fortunately, Granary allows \toolname{Irrigator} to instrument specific kernel functions---in this case memory deallocators---and detect when the kernel frees module-allocated memory. \toolname{Irrigator} also interposes on specific kernel memory allocators (e.g. for the \texttt{sk\_buff} data structures), which are allocated by the kernel but deallocated by modules (e.g. network device drivers deallocate \texttt{sk\_buff} data structures).

%We also use probe-based instrumentation to assign module ownership to kernel-allocated \texttt{sk\_buff} data structures, which are allocated by the kernel but deallocated by network device driver modules.

%: \begin{inparaenum}[i)]
%	\item the instrumentation tool is only interested in that particular function; or
%	\item when the instrumentation tool needs visibility into that function
%\end{inparaenum}
...
%Like KernInst \cite{KernInst,AnyWhereAnyTimeDBT}, Granary can take over the execution of individual kernel functions. 
...
% Instrumenting invidual functions, called \emph{probe-based instrumentation}, giv \cite{KProbes}.
...
%Probe-based instrumentation provides visibility into specific kernel events (e.g. memory allocations) to instrumentation tools that don't instrument the whole kernel (e.g. module-only, \Cref{fig:module_probe}). One example tool that combines module-only and probe-based instrumentation is 
...
%\subsection{On-Demand Instrumentation}
%
