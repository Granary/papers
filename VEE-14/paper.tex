...
The \texttt{RCUdbg} tool detects the the following bugs: \begin{enumerate}
	\item Reads through an RCU-dereferenced pointer outside of a read-side critical section (\Cref{fig:rcu_existence_bug}).
	\item Reads through an RCU-assigned pointer that don't first use \texttt{rcu\_dereference} (\Cref{fig:rcu_deref_bug}).
	\item Reads through fields of RCU-protected data structures that should be accessed via \texttt{rcu\_dereference} (\Cref{fig:rcu_assign_bug}).
	\item Reads through an RCU-dereferenced pointer in the wrong read-side critical section.
	\item Writes through an RCU-dereferenced pointer.
	\item Writes through an RCU-assigned pointer.
\end{enumerate}
