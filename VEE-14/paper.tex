...
\section{Scope-Driven Specialization}\label{sec:what}

Some instrumentation tools have a limited scope. That is, they are made to solve a specific problem within a specific domain. For example, Granary's Guardrail tool is concerned with finding bugs in Linux device drivers \cite{Guardrail}. Guardrail records and analyzes traces of device driver code to find bugs. However, Guardrail's analysis can be heavyweight, and so limiting the analysis to only device driver code results in better performance. Granary allows Guardrail to set the granularity of instrumentation to only instrument a specific module, while leaving the rest of the kernel to execute without overhead.
...
Guardrail is an example of how tools can choose an instrumentation granularity that better matches their scope and needs. That is, a Granary tool can choose to instrument some code, while leaving other code to execute natively. This is useful when the instrumentation in question has a high overhead or when only a subset of the kernel's code is of interest. \Cref{tab:granularities} provides a high-level overview of the different granularities of instrumentation supported by Granary. Granary tools, however, are not restricted to only the Granularities listed in the table.
