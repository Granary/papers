...
\begin{table*}[ht!]
\scriptsize
\begin{tabularx}{\linewidth}{| c | >{\raggedright}m{0.3\linewidth} | X |}
\hline
\bf Scope & \bf Description & \bf Example Tool \\
\hline
...
\hline
Module & All code from a specific kernel module or driver is instrumented. Granary detaches when module code invokes kernel code, and re-attaches when kernel code invokes module code. All non-module kernel code executes with zero overhead. & TODO \\
\hline
...
\hline
\end{tabularx}
\caption{\label{tab:what_kinds}Different kinds to instrumentation strategies supported by Granary.}
\end{table*}
...
Granary allows tool developers to precisely specify the scope or granularity of instrumentation. For example, Granary can instrument all kernel code, specific module code, or only specific function code.
