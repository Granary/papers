...
\paragraph{Ahead-of-time Translation} 
...
The method used to pad basic blocks so as to align specific instructions must be transparent (i.e. not change the execution behavior). The typical approach to alignment is to introduce \texttt{nop} instructions, or \texttt{jmp} instructions which jump over the padding. Adding extra instructions, however, introduces added latency to the fetch/decode/execute cycle. Therefore, when possible, Granary pads instructions using otherwise ignored instruction prefixes. For example, Granary aligned patchable conditional jumps using static branch prediction hint prefixes, which are ignored by recent x86 processors \cite{AgnerMicroarchitecture}).
