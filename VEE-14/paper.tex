...
Control-flow graphs are reported in terms of basic blocks (nodes) and edges (control flow transfers). The reporting operates by iterating over each basic block and performing the following for each basic block: \begin{enumerate*}
	\item Decode the native instructions associated with the basic block. The location and number of instructions to decode is found in the basic block's meta-data (\Cref{sec:metadata}).
...
%A single-element cache is also maintained that records the most recent indirect branch target. Most indirect CTIs in the kernel only have one target, and so once the target has been permanently recorded, all future executions hit the cache.

%As an optimization, the indirect target recorders maintain a one-element cache of the most recent target of a given indirect branch


% Basic blocks in the graph can be labeled with execution counts, and control-flow edges between basic blocks can also be labeled with execution counts. Counting basic block and edge executions enables Granary's \toolname{cfg} tool implicitly provides code coverage and profiling information. \Cref{fig:lmbench} evaluates how profiling information from Granary's \toolname{cfg} tool is integrated with Granary's ahead-of-time translation optimization (\Cref{sec:aot}).
...
% with eachother and operate on RCU-protected pointers.

%There are two kinds of RCU-protected pointers: read-only pointers, and read/write pointers. Read-only pointers can only be read from within RCU read-side critical sections, delimited by  \texttt{rcu\_\linebreak[0]read\_\linebreak[0]lock} and \texttt{rcu\_\linebreak[0]read\_\linebreak[0]unlock}. Read/write pointers can be written to 



%Code operating on RCU-protected pointers must use specific API functions: , and By assuming that these functions are correctly used some of the time, we can 

%Therefore, a use of \texttt{rcu\_\linebreak[0]dereference} implies that the pointer being read should be RCU-protected, and the the value returned should also be RCU-protected. Without an \emph{a priori} understanding of the data structures being operated on, these two API functions give \toolname{RCUdbg} enough information about RCU protected pointers to find bugs.

%Without a prior understanding of what pointers are RCU-protected pointers, we can assume that any pointer operated on by one the aforementioned API functions is an RCU-protected pointer.

%Without an \emph{a priori} understanding of the structure of the data being syncrhonized using RCU, we can conservatively assert that RCU-protected pointers.
...
%	\item RCU defines how writers can update RCU-protected pointers within a shared data structure. Modification of non-RCU-protected pointer fields within an RCU-protected data structure is beyond the scope of RCU's correctness guarantees.
...
\begin{figure}[ht!] %
\subfloat[\label{fig:rcu_existence_bug}Despite correctly obtaining a pointer to a shared data structure using \texttt{rcu\_dereference} within a read-side critical section, the data pointed to by \texttt{elm} is not guaranteed to exist after the read-side critical section ends.]{\fbox{\usebox\rcuexistencebug}} %

\subfloat[\label{fig:rcu_deref_bug}Despite the call to \texttt{rcu\_dereference}, \texttt{list\_head} is not guaranteed to be equivalent to \texttt{elm} because a writer might have concurrently changed the value of the \texttt{list\_head} pointer. That is, if \texttt{elm} is non-\texttt{NULL}, then the data pointed to by \texttt{elm} is guaranteed to exist for the remainder of the read-side critical section. However, the value of \texttt{list\_head} is \emph{not} guaranteed to remain consistent--even within read-side critical sections--and so \texttt{list\_head} might point to a \texttt{NULL} pointer when \texttt{elm} does not.]{\fbox{\usebox\rcuderefbug}} %

\subfloat[\label{fig:rcu_assign_bug}This example shows the code for a writer thread adding a new element into the second position of an RCU-protected linked list. The code for reader threads is also shown, where readers iterate over the list by traversing \texttt{next} pointers and sum up the \texttt{data} fields of each visited list element. Unfortunately, the access of \texttt{elm->next} results in undefined behavior. This is because the next element of the list is only guaranteed to be fully constructed within the read-side critical section if the reader executes \texttt{rcu\_dereference(elm->next)}. Therefore, the first read of \texttt{elm->next} might yield a pointer to partially constructed data.  Because that data is not guaranteed to be fully constructed, the next read of \texttt{elm->next} (i.e. \texttt{elm->next->next}) might read an invalid pointer from uninitialized memory. Treating the value of uninitialized memory as a valid pointer could trigger a kernel panic or silent data corruption.]{\fbox{\usebox\rcuassignderefbug}} %
\caption{\label{fig:rcu_bugs}Example RCU bugs related to misuses of RCU-protected pointers.}
\end{figure}

\toolname{RCUdbg} is primarily concerned with finding bugs related to RCU-protected pointer misuses by reader threads (both inside and outside of read-side critical sections). As mentioned above, RCU bugs sometimes manifest as use-after-free bugs. Unfortunately, a pure use-after-free-based checking approach is insufficient. \Cref{fig:rcu_assign_bug} is an example where a use-after-free based checking approach will miss an RCU-protected pointer misuse because the bug does not manifest as a use-after-free. \Cref{fig:rcu_existence_bug} is an example of an RCU usage bug that can manifest as a use-after-free bug, but only if one is unlucky enough to observe a very specific thread schedule. That is, suppose that execution is interrupted after \texttt{rcu\_read\_unlock}, but before the access to \texttt{elm->data}. If \texttt{elm} is freed before the interrupted code resumes its execution, then the access to \texttt{elm->data} is a use-after-free bug. This kind of bug is a time bomb waiting to go off. Fortunately, \toolname{RCUdbg} catches this bug and others in a scheduling-independent way.
...
\begin{itemize}[leftmargin=3.2em]
	\item[\bf B1] Reads through an RCU-dereferenced pointer outside of a read-side critical section (\Cref{fig:rcu_existence_bug}).
	\item[\bf B2] Reads through an RCU-dereferenced pointer in the wrong read-side critical section.

	\item[\bf B2] Reads through an RCU-assigned pointer that don't first use \texttt{rcu\_dereference} (\Cref{fig:rcu_deref_bug}).
	\item[\bf B3] Reads through fields of RCU-protected data structures that should be accessed via \texttt{rcu\_dereference} (\Cref{fig:rcu_assign_bug}).
...
\toolname{RCUdbg} gains visibility on RCU operations in three ways: \begin{enumerate}
	...
	\item The annotated RCU API functions are specialized using Granary's function wrapping feature (\Cref{sec:function_wrapper}). Interposing on these functions allows \toolname{RCUdbg} to add an address watchpoint to the address returned by \texttt{rcu\_\linebreak[0]dereference}, and to correlate uses of \texttt{rcu\_\linebreak[0]dereference} and \texttt{rcu\_\linebreak[0]assign\_\linebreak[0]pointer}.
...
\subfloat[Fast operations]{\epsfig{file=charts/lmbench_fast.eps,height=14em}}
\subfloat[Fork operations]{\epsfig{file=charts/lmbench_fork.eps,height=14em}}\\
\subfloat[Fast operations (Normalized)]{\epsfig{file=charts/lmbench_fast_norm.eps,height=14em}}
\subfloat[Fork operations (Normalized)]{\epsfig{file=charts/lmbench_fork_norm.eps,height=14em}}
\caption{\label{fig:lmbench}Microbenchmark results from \texttt{lmbench} that shows the effect of Granary's code cache optimizations (\Cref{sec:optimize}).}
\end{figure*}

\section{Evaluation}\label{sec:eval}

Before examining applications that are enabled by Granary, \Cref{sec:microbench} evaluates the overhead of the baseline instrumentation. Then, \Cref{sec:macrobenchmark} does something with the evaluation TODO.
...
% when profiling data from Granary's \toolname{cfg} tool are used to predict and potentially exchange the targets of conditional branches.
